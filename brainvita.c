...
#define YES 1
#define NO 0
int x=320,y=100,marble=32,marble_colour=12;
void *p;
size_t area;
...
int GetBoard(int X,int Y,int element);
...
void Marble(int x,int y,int c)
{
	...
	if(c!=0)
	{
		if(c==15){setcolour(7);setfillstyle(1,7);}
		else{setfillstyle(1,15);setcolour(15);}
...
{
	int i;
	char pattern[8]={0xfe,0xbc,0xda,0xe1,0xe2,0xc0,0x88,0x00};
...
void Background()
{
	...
	setcolour(15);
	outtextxy(42,100,". or - : colour");
	outtextxy(42,120,"^T : Up");
	outtextxy(42,140,"_: : Down");
	...
	outtextxy(523,100,"Lo move around");
...
// when ENTER pressed check for condition & perform task
...
int MakeMove(int X,int Y)
{
	...
	if(key==0x1d00)
	{
		if(GetBoard(X+80)==0 && GetBoard(X+40,Y)==1)
		{
			Marble(X,Y,0);
			Setcolour(X,Y,0);
			X+=40;
...
// Handles all the function & perform desired move

void Init()
{
	int i,j,e=1;
	char ch;
	setwritemode(XDR_PUT);
	setcolour(15);
	while(e)
	{
		setfillstyle(1,11);
		...
		if(GetXY(x,y)==1)
		{
			switch(ch)
			{
				...
				case ESC	:e=0;break;
				case '+' : marble_colour++;
				         if(marble_colour>15) marble_colour=2;
						 DrawBoard();
						 break;
				case (-) :marble_colour--;
				         if(marble_colour>15) marble_colour=15;
						 DrawBoard();
						 break;
...
int finish()
{
	...
	switch(f)
	{
		...
		case 7 :outtextxy(230,180,"  Try Again !  ");break;
	}
	getch();
...
void LCD(int left,int top,int NUM)
{
	...
	switch(NUM)
	{
		...
		case 3:
			line(left,top,left+10,top);
			line(left+11,top+2,left+11,top+9);
			line(left+1,top+11,left+11,top+18);
			line(left+11,top+11,left+11,top+18);
			line(left,top+20,left+10,top+20);
			break;
		case 4:
...
int save_game()
{
	...
	gotoxy(20,2);printf("File name: ");
	scanf("%s",fname);
	if( ( fp = fopen ( fname, "%s ") ) == NULL ) return 0;
